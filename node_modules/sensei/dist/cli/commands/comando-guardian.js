...
export function comandoGuardian(aplicarFlagsGlobais) {
    return new Command('guardian').description('Gerencia e verifica a integridade do ambiente do Sensei.')
        .allowUnknownOption(true).allowExcessArguments(true).option('-a, --accept-baseline', 'Aceita o baseline atual como o novo baseline de integridade').option('-d, --diff', 'Mostra as diferenças entre o estado atual e o baseline').option('--full-scan', 'Executa verificação sem aplicar GUARDIAN_IGNORE_PATTERNS (não persistir baseline)').option('--json', 'Saída em JSON estruturado (para CI/integracoes)').action(async function (opts) {
        try {
                    ...
                    switch (statusNorm) {
                        case IntegridadeStatus.Ok:
                            if (opts.json)
                                console.log(JSON.stringify({
                                    status: 'ok',
                                    cacheDiffHits: globalThis.__SENSEI_DIFF_CACHE_HITS__ || 0
                                }));
                            else
