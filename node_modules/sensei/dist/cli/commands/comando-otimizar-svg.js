...
export function comandoOtimizarSvg(aplicarFlagsGlobais) {
    return new Command('otimizar-svg')
        .description('Otimiza SVGs do projeto usando o otimizador interno (svgo-like).')
        ...
        .option('--include <padrao>', 'Glob pattern a INCLUIR (pode repetir ou separar por vírgula/espaço)', (val, prev) => {
        prev.push(val);
        return prev;
    }, [])
        .option('--exclude <padrao>', 'Glob pattern a EXCLUIR adicionalmente (pode repetir ou separar por vírgula/espaço)', (val, prev) => {
        prev.push(val);
        return prev;
    }, [])
        ...
        try {
            ...
            for (const e of entries) {
                ...
                if (!dry) {
                    ...
                    optimized++;
                    log.info(`${relPath} — ${formatBytes(opt.originalBytes)} → ${formatBytes(opt.optimizedBytes)} (−${formatBytes(saved)})`);
                }
                else {
                    log.info(`[dry] ${relPath} — ${formatBytes(opt.originalBytes)} → ${formatBytes(opt.optimizedBytes)} (−${formatBytes(saved)})`);
                }
            }
            if (!candidates) {
            ...
            log.info(`Candidatos: ${candidates} | Economia potencial: ${formatBytes(savedBytes)} | Total de SVGs lidos: ${total}`);
            if (write) {
                log.sucesso(`Otimização aplicada em ${optimized}/${candidates} arquivos.`);
            }
            else {
            ...
            log.erro(`Falha ao otimizar SVGs: ${err instanceof Error ? err.message : String(err)}`);
