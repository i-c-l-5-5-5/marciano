...
export function registrarComandoReverter(program) {
    ...
    })).addCommand(new Command('arquivo').description('Reverte todos os moves de um arquivo específico').argument('<arquivo>', 'Caminho do arquivo para reverter').action(async (arquivo) => {
        try {
            await mapaReversao.carregar();
    ...
    })).addCommand(new Command('move').description('Reverte um move específico pelo ID').argument('<id>', 'ID do move para reverter').action(async (id) => {
        try {
            await mapaReversao.carregar();
