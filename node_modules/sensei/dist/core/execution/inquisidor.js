...
export async function prepararComAst(entries, baseDir) {
    ...
    const cache = globalStore.__SENSEI_AST_CACHE__ || new Map();
    if (!globalStore.__SENSEI_AST_CACHE__)
        globalStore.__SENSEI_AST_CACHE__ = cache;
    const metricas = globalStore.__SENSEI_METRICAS__ || {
        parsingTimeMs: 0,
        cacheHits: 0,
        cacheMiss: 0
    };
    ...
    return Promise.all(entries.map(async (entry) => {
        ...
        if (entry.content && EXTENSOES_COM_AST.has(extEfetiva)) {
            ...
            try {
                if (!ast) {
                    ...
                    if (parsed && typeof parsed === 'object') {
                        ...
                        if (inNodeModules) {
                            ...
                            const lista = globalStore2.__SENSEI_PARSE_ERROS__ || [];
                            try {
                ...
                const lista = globalStore.__SENSEI_PARSE_ERROS__ || [];
                lista.push(ocorrenciaParseErro({
...
export async function iniciarInquisicao(baseDir = process.cwd(), options = {}, tecnicas) {
        ...
        onProgress: msg => {
            try {
                ...
                if (progressData.tipo === 'diretorio') {
                    ...
                    const SAMPLE_MAX = 5;
                    if (!g.__SENSEI_DIR_SAMPLES__)
                        g.__SENSEI_DIR_SAMPLES__ = [];
    ...
    if (!skipExec) {
        ...
        if (!tecnicasEfetivas) {
            ...
            tecnicasEfetivas = registroAnalistas;
        }
    ...
    const parseErros = globalThis.__SENSEI_PARSE_ERROS__ || [];
    if (parseErros.length) {
        globalThis.__SENSEI_PARSE_ERROS_ORIGINAIS__ = parseErros.length;
